<x-filament::page>
    <style>
        ...
        .report-dashboard .badge-cash {
            background: linear-gradient(135deg, #10b981 0%, #059669 100%);
        }
        
        .report-dashboard .badge-qris {
            background: linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%);
        }
        
        .report-dashboard .badge-debit {
            background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%);
        }
        
        .report-dashboard .badge-credit {
        ...
        /* Text colors yang match dan keren */
        .report-dashboard .text-premium-dark {
            color: #1e293b;
        }
        
        .report-dashboard .text-premium-light {
            color: #64748b;
        }
        
        .report-dashboard .text-premium-blue {
            color: #3b82f6;
        }
        
        .report-dashboard .text-premium-purple {
            color: #8b5cf6;
        }
        
        .report-dashboard .text-premium-green {
            color: #10b981;
        }
        
        .report-dashboard .text-premium-orange {
            color: #f59e0b;
        }
        
        /* Hover effects */
        .report-dashboard .hover-lift {
            transition: all 0.3s ease;
        }
        
        .report-dashboard .hover-lift:hover {
    ...
    <div class="report-dashboard space-y-6">
        ...
        <div class="glass-effect rounded-2xl shadow-xl p-6 border border-gray-100 backdrop-blur-sm">
            ...
            <div class="flex justify-end mt-4">
                <button wire:click="exportExcel" 
                        class="bg-gradient-to-r from-green-500 to-green-600 text-white px-5 py-2 rounded-xl hover:shadow-lg transition-all flex items-center gap-2">
                    📊 Export Excel
                </button>
            </div>
        </div>

        {{-- Statistik Card Premium dengan Animasi --}}
            ...
            <div class="stat-card gradient-blue rounded-2xl p-6 shadow-xl cursor-pointer">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div class="text-right">
                        ...
                        <div class="text-2xl font-bold text-white number-counter">Rp {{ number_format($totalPenjualan, 0, ',', '.') }}</div>
                    </div>
                </div>
            ...
            <div class="stat-card gradient-orange rounded-2xl p-6 shadow-xl cursor-pointer">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div class="text-right">
                        ...
                        <div class="text-2xl font-bold text-white number-counter">{{ number_format($totalTransaksi) }}</div>
                    </div>
                </div>
            ...
            <div class="stat-card gradient-purple rounded-2xl p-6 shadow-xl cursor-pointer">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div class="text-right">
                        ...
                        <div class="text-2xl font-bold text-white number-counter">{{ number_format($totalProdukTerjual) }}</div>
                    </div>
                </div>
            ...
            <div class="stat-card gradient-green rounded-2xl p-6 shadow-xl cursor-pointer">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div class="text-right">
                        ...
                        <div class="text-2xl font-bold text-white number-counter">Rp {{ number_format($rataRataTransaksi, 0, ',', '.') }}</div>
                    </div>
                </div>
        ...
        <div class="glass-effect rounded-2xl shadow-xl p-6 border border-gray-100 backdrop-blur-sm">
            ...
            <canvas id="salesChart" height="80" style="max-height: 300px; width: 100%;"></canvas>
        </div>

        {{-- Top Produk & Ringkasan Premium --}}
        <div class="grid grid-cols-2 gap-6">
                    ...
                    @forelse($this->topProducts as $index => $product)
                        <div class="group flex justify-between items-center p-4 bg-white rounded-xl hover:shadow-lg transition-all cursor-pointer border border-gray-100">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 flex items-center justify-center font-bold text-white shadow-md">
                                    {{ $index + 1 }}
                                </div>
                            ...
                            <div class="flex items-center gap-4">
                                <div class="w-32 h-2 bg-gray-200 rounded-full overflow-hidden">
                                    <div class="h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full transition-all duration-500 group-hover:scale-x-110" style="width: {{ min(100, ($product->total_terjual / max($this->topProducts->first()->total_terjual, 1)) * 100) }}%"></div>
                                </div>
                                <span class="text-premium-purple font-bold">{{ $product->total_terjual }} terjual</span>
                            </div>
                        </div>
                    @empty
            ...
            {{-- Ringkasan Metode Pembayaran Premium --}}
            <div class="glass-effect rounded-2xl shadow-xl p-6 border border-gray-100 backdrop-blur-sm">
                <div class="flex items-center gap-3 mb-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="group cursor-pointer">
                        <div class="flex justify-between mb-2">
                            ...
                            <span class="text-premium-light">{{ $cashCount }} transaksi ({{ round(($cashCount / $total) * 100) }}%)</span>
                        </div>
                    ...
                    <div class="group cursor-pointer">
                        <div class="flex justify-between mb-2">
                            ...
                            <span class="text-premium-light">{{ $qrisCount }} transaksi ({{ round(($qrisCount / $total) * 100) }}%)</span>
                        </div>
                        <div class="w-full h-3 bg-gray-200 rounded-full overflow-hidden">
                    ...
                    <div class="group cursor-pointer">
                        <div class="flex justify-between mb-2">
                            ...
                            <span class="text-premium-light">{{ $debitCount }} transaksi ({{ round(($debitCount / $total) * 100) }}%)</span>
                        </div>
                        <div class="w-full h-3 bg-gray-200 rounded-full overflow-hidden">
                    ...
                    <div class="group cursor-pointer">
                        <div class="flex justify-between mb-2">
                            ...
                            <span class="text-premium-light">{{ $creditCount }} transaksi ({{ round(($creditCount / $total) * 100) }}%)</span>
                        </div>
                        <div class="w-full h-3 bg-gray-200 rounded-full overflow-hidden">
        ...
        <div class="glass-effect rounded-2xl shadow-xl p-6 border border-gray-100 backdrop-blur-sm">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="text-sm text-premium-light bg-gray-100 px-3 py-1 rounded-full">
                    ...
                    <tbody>
                        @forelse($transactions as $transaction)
                            <tr class="table-row border-b cursor-pointer hover:bg-gray-50">
                                <td class="p-4 font-semibold text-premium-purple">{{ $transaction->invoice_number }}</td>
                                <td class="p-4 text-premium-light">{{ $transaction->created_at->format('d/m/Y H:i') }}</td>
                                <td class="p-4 font-medium text-premium-dark">{{ $transaction->customer_name }}</td>
